...
2. Ensure the repo's `public_html/` directory is deployed as the document root (or symlinked there). The included `.htaccess` routes `/api/*` calls to `index.php`, while the portal (`/`) and static assets such as `/admin/` load directly.
...
## Main Portal ( `/` )
When someone visits the bare domain they now see a focused control room:

- Inputs for the API base URL, admin token, and default product code (persisted locally per browser).
- Built-in forms for issuing, validating, activating, and deactivating licenses without touching curl.
- A live console log that captures every request/response for auditing.
- An API reference grid with copy-ready `curl` commands for the four POST endpoints, explaining required fields and headers.

Keep the portal behind an authentication layer (SiteGround password protection or VPN) so only trusted operators can access it.
